...
class PathPlanner(object):
    ...
    def __init__(self, group_name):
        ...
        self._planning_scene_publisher = rospy.Publisher(
            '/collision_object', CollisionObject, queue_size=10)

        self._internal_collision_object_list = []
        ...
        # Sleep for a bit to ensure that all initialization has finished
        rospy.sleep(0.5)
    ...
    def plan_to_pose(self, target, orientation_constraints, vel_factor=0.5, acc_factor=0.5):
        """
        ...
        vel_factor, acc_factor: Float of planned trajectory velocity/acceleration scale (0, 1]
        ...
        self._group.set_max_velocity_scaling_factor(vel_factor)
        self._group.set_max_acceleration_scaling_factor(acc_factor)
    ...
    def add_box_obstacle(self, size: List, name: str, pose: PoseStamped):
        """
        Adds a rectangular prism obstacle to the planning scene

        Inputs:
        size: 3x' plain list; (x, y, z) size of the box (in the box's body frame)
        ...
        co.id = name
        self._internal_collision_object_list.append(name)
    ...
    def remove_obstacle(self, name):
        ...
        co.id = name
        self._internal_collision_object_list.remove(name)
        self._planning_scene_publisher.publish(co)

    def clear_obstacle(self):
        obstacle_list = self._internal_collision_object_list.copy()
